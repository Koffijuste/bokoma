...
class PaymentService {
  ...
  initializePayment = async ({
    ...
    description,
    customer,
    orderId,           // ✅ Ajout pour construire les URLs de retour
    payment_method = 'OM_CI',
  }) => {
    const token     = await this.getAccessToken();
    // ✅ Arrondir au multiple de 5 (règle CinetPay XOF)
    const safeAmount = Math.round(Number(amount) / 5) * 5;

    // ✅ URLs DISTINCTES pour succès et échec
    const successUrl = `${this.clientUrl}/payment/success?orderId=${orderId}`;
    const failedUrl  = `${this.clientUrl}/payment/failed?orderId=${orderId}`;
    ...
    const payload = {
      ...
      amount:                safeAmount,
      success_url:           successUrl,   // ✅ URL succès
      failed_url:            failedUrl,    // ✅ URL échec (différente !)
    ...
    throw new AppError(data.description || 'Erreur initialisation paiement CinetPay', 400);
  }
