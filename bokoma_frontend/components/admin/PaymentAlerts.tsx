...
export function PaymentAlerts() {
  ...
  const fetchPaymentAlerts = useCallback(async () => {
    try {
      ...
      const [pendingRes, failedRes, successRes] = await Promise.allSettled([
        apiClient.get('/orders/payments/pending', { signal: AbortSignal.timeout(10000) }),
        apiClient.get('/orders/payments/failed', { 
          params: { hours: 2 },
          signal: AbortSignal.timeout(10000) 
        }),
        apiClient.get('/orders/payments/success', { 
          params: { hours: 2 },
          signal: AbortSignal.timeout(10000) 
        }),
      ]);
  ...
  useEffect(() => {
    fetchPaymentAlerts();
    const interval = setInterval(fetchPaymentAlerts, 15000);
  ...
  const handleRejectPayment = async (orderId: string) => {
    ...
    try {
      await apiClient.post(`/orders/${orderId}/payment/reject`, {
  ...
  const getTimeRemaining = (createdAt: string, expiresAt?: string): { minutes: number; urgent: boolean } => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Aucun paiement en attente ou en erreur
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Surveillance temps réel • Rejet automatique après 30 min
            </p>
          </div>
        </div>
        <Button
                      ...
                      {timeInfo?.urgent && (
                        <div className="mt-2 flex items-center gap-1.5 text-xs text-red-600">
                          ...
                          <span className="font-medium">
                            {timeInfo.minutes === 0 
                              ? 'Expiré ! Rejet automatique imminent' 
                              : `Expire dans ${timeInfo.minutes} min`}
                          </span>
                        </div>
                      )}
                    </div>
      ...
      <div className="p-4 border-t border-border bg-muted/30">
        <Link 
          href={`${ROUTES.ADMIN.ORDERS}?filter=payment`}
