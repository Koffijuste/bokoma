...
import {
  ...
  LayoutDashboard, Bell, ChevronDown, Sparkles, Settings, Crown, Star
} from 'lucide-react';
...
export function Header() {
  ...
  // Scroll detection avec hide on scroll down
  ...
  if (!mounted) {
    return (
      <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/50 h-16">
        <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-accent to-accent/70 flex items-center justify-center">
                            ...
                            {user.role && (
                              <div className="mt-3 inline-flex items-center gap-1 px-2 py-1 rounded-full bg-accent/10 text-accent text-xs font-medium">
                                {user.role === 'admin' ? (
                                  ...
                                  <><Star className="w-3 h-3" /> Gestionnaire</>
                                ) : (
